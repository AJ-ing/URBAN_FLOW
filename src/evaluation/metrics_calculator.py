...
class MetricsCalculator:
    ...
    def _init_state(self) -> None:
        ...
        # Departed-vehicle ledger: (depart_time, wait_time, travel_time)
        self.departed_vehicles: List[Tuple[float, float, float]] = []

        # Frame / clock counters
        self.tick_count: int = 0
        self.current_time: float = 0.0

        # Per-direction previous crossed counts (unused in current impl
        # but kept for future delta-based detection if needed).
        ...
        # Set of vehicle ids already recorded as departed
    ...
    def update(
        ...
        for vehicle in vehicles_iter:
            ...
            # 4. Detect newly departed vehicles
    ...
    def _calc_avg_wait(self) -> float:
        ...
        if not self.departed_vehicles:
            return 0.0
        total: float = sum(w for (_, w, _) in self.departed_vehicles)
    ...
    def _calc_avg_travel(self) -> float:
        ...
        if not self.departed_vehicles:
            return 0.0
        total: float = sum(tt for (_, _, tt) in self.departed_vehicles)
        return total / len(self.departed_vehicles)
